...
class EndpointTest(unittest.TestCase):
    ...
    def test_FizzbuzOk1(self):
        response = self.client.post('/v1/fizzbuzz?int1=2&int2=3&limit=10&str1=aaz&str2=osjpd')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json['success'], True)
        self.assertEqual(response.json['result'], [1, 'aaz', 'osjpd', 'aaz', 5, 'aazosjpd', 7, 'aaz', 'osjpd', 'aaz'])

    def test_FizzbuzOk2(self):
        response = self.client.post('/v1/fizzbuzz?int1=3&int2=5&limit=15&str1=fizz&str2=buzz')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json['success'], True)
    ...
    def test_MetricsGetOne(self):
        self.client.post('/v1/fizzbuzz?int1=3&int2=5&limit=15&str1=fizz&str2=buzz')
        response = self.client.get('/v1/metrics')
        self.assertEqual(response.status_code, 200)
    ...
    def test_MetricsGetTen(self):
        for i in range(10):
            self.client.post('/v1/fizzbuzz?int1=2&int2=3&limit=10&str1=test&str2=me')       
        response = self.client.get('/v1/metrics')
        self.assertEqual(response.status_code, 200)
